...
// Mock options data until paid plan — real prices shown above
const fetchOptionsChain = async (ticker, basePrice) => {
  const rows = [];
  const baseStrike = Math.floor(basePrice / 5) * 5;
  const allBuckets = [...QUICK_BUCKETS, ...MONTHLY_BUCKETS];
  for (let i = 10; i >= -10; i--) {
    const strike = baseStrike + i * 5;
    const cells = allBuckets.map((bucket) => {
      const daysToExp = parseInt(bucket);
      const isOTM = i !== 0;
      const volume = Math.floor(Math.random() * 5000 + 500);
      const oi = Math.floor(Math.random() * 10000 + 1000);
      const premium = Math.random() * 2000000 + 100000;
      const isAskSide = Math.random() > 0.3;
      const isSweep = volume > oi * 0.4 && Math.random() > 0.6;
      const isGoldenSweep = premium > 500000 && isOTM && daysToExp <= 7 && isSweep && isAskSide;
      ...
      return { strike, expiration: bucket, callOrPut, totalPremium: premium, bullishPremium: isBullish ? premium : 0, bearishPremium: isBullish ? 0 : premium, volume, openInterest: oi, isGoldenSweep, scoreContribution: score, isBullish };
    });
    const totalPremium = cells.reduce((s, c) => s + c.totalPremium, 0);
    const bullishTotal = cells.reduce((s, c) => s + c.bullishPremium, 0);
    rows.push({ strike, cells, totalPremium, isBullish: bullishTotal >= totalPremium / 2 });
  }
  return rows;
};

// Fetch real price using free tier previous close endpoint
const fetchQuote = async (ticker) => {
  try {
    ...
    if (data.results && data.results.length > 0) {
      ...
      const price = r.c;
      const priceChange = ((r.c - r.o) / r.o * 100);
...
const SmartMoneyDashboard = () => {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    // Fetch real prices one at a time to respect rate limit
    for (const name of tickers) {
      const tickerInfo = TICKERS[name];
      if (!tickerInfo) continue;
      const symbol = name === 'SPX' ? 'SPY' : tickerInfo.symbol; // use SPY price for SPX approximation
    ...
    // Build heatmap rows with mock data but real base prices
    for (const name of tickers) {
      const tickerInfo = TICKERS[name];
      if (!tickerInfo) continue;
      const basePrice = newQuotes[name]?.price || tickerInfo.basePrice;
      const rows = await fetchOptionsChain(name, basePrice);

      const quickRows = rows.map(r => ({ ...r, cells: r.cells.filter(c => QUICK_BUCKETS.includes(c.expiration)) }));
      const monthlyRows = rows.map(r => ({ ...r, cells: r.cells.filter(c => MONTHLY_BUCKETS.includes(c.expiration)) }));

      allData[name] = quickRows;
      allScores[name] = calculateSmartMoneyScore(quickRows);

      if (name === customTicker) setExpandedMonthlyData(monthlyRows);

      rows.forEach(row => {
        row.cells.forEach(cell => {
          if (cell.isGoldenSweep) {
            allGoldenSweeps.push({
              ticker: name, strike: cell.strike, type: (cell.callOrPut || 'call').toUpperCase(),
              expiration: cell.expiration, premium: cell.totalPremium,
              score: cell.scoreContribution, grade: getGrade(cell.scoreContribution),
